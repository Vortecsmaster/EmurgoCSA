...
import { fromHex, getAddressDetails, toHex } from "../utils/mod.js";
export class Blockfrost {
    ...
    async getUtxos(address) {
        ...
        let page = 1;
        while (true) {
            const pageResult = await fetch(`${this.url}/addresses/${credentialBech32}/utxos?page=${page}`, { headers: { project_id: this.projectId } }).then((res) => res.json());
            if (pageResult.error) {
    ...
    async getUtxosWithUnit(address, unit) {
        ...
        let page = 1;
        while (true) {
            const pageResult = await fetch(`${this.url}/addresses/${credentialBech32}/utxos/${unit}?page=${page}`, { headers: { project_id: this.projectId } }).then((res) => res.json());
            if (pageResult.error) {
                if (pageResult.status_code === 404) {
                    return [];
                }
    ...
    async getUtxosByOutRef(outRefs) {
        ...
        const utxos = await Promise.all(queryHashes.map(async (txHash) => {
            const result = await fetch(`${this.url}/txs/${txHash}/utxos`, { headers: { project_id: this.projectId } }).then((res) => res.json());
    ...
    async getDelegation(rewardAddress) {
        const result = await fetch(`${this.url}/accounts/${rewardAddress}`, { headers: { project_id: this.projectId } }).then((res) => res.json());
        if (!result || result.error) {
        ...
        return (await Promise.all(result.map(async (r) => ({
            ...
            scriptRef: r.reference_script_hash &&
                (await (async () => {
                    ...
                    const { cbor: script } = await fetch(`${this.url}/scripts/${r.reference_script_hash}/cbor`, { headers: { project_id: this.projectId } }).then((res) => res.json());
                    return {
